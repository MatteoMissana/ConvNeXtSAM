...
class ConvNextBlock1(nn.Module):      # dw nel mezzo
    def __init__(self, c1, dim, k = 3, p = 1, shortcut=True, drop_path=0., layer_scale_init_value=0):
        super().__init__()
        c_ = 4 * dim
        self.add = shortcut
...
class ConvNextBlock(nn.Module):
    def __init__(self, c1, c2, n, k=7, p=3, dw_mid=False, shortcut=True, drop_path=0., layer_scale_init_value=1e-6):
        super().__init__()
        if not dw_mid:
...
class SpatialAttDownSample(nn.Module):
    def __init__(self, c1, c2, k, s, p):
        super().__init__()
        self.layer = nn.Conv2d(c1, c2, k, s, p)
        self.SA = cbam.SpatialGate()
    def forward(self,x):
        x = self.layer(x)
        x = self.SA(x)
        return x
...
class ConvNextBlockCSP(nn.Module):
    def __init__(self, c1, c2, n, k=7, p=3, dd = False, dw_mid=False, shortcut=True, drop_path=0., layer_scale_init_value_csp=0, layer_scale_init_value_in=0, e=0.5): #paper LS = 1e-6
        super().__init__()
        self.act = nn.GELU()
        c_ = int(c2 * e)  # hidden channels
        self.norm = LayerNorm(c1)
